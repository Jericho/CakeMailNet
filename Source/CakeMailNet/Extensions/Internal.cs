...
using static CakeMailNet.Utilities.DiagnosticHandler;
...
namespace CakeMailNet
{
	...
	internal static class Internal
	{
		...
		internal static async Task<(bool IsError, string Message)> GetErrorMessageAsync(this HttpResponseMessage message)
		{
			...
			/*
				In case of an error, the CakeMail API returns a JSON string that looks like this:
				{
					"detail" :
					[
						{
							"loc" : [ "body", "grant_type" ],
							"msg" : "value is not a valid enumeration member; permitted: 'password'",
							"type" : "type_error.enum",
							"ctx" : { "enum_values" : [ "password" ] }
						}
					]
				}
			*/
			...
			if (!string.IsNullOrEmpty(responseContent))
			{
				try
				{
					...
					{
						var foundDetails = rootJsonElement.TryGetProperty("detail", out JsonElement jsonDetails);

						// Check for the presence of property called 'detail' (it's spelled singular, but it contains an array of multiple errors)
						if (foundDetails && jsonDetails.ValueKind == JsonValueKind.Array)
						{
							var errors = jsonDetails.EnumerateArray()
								.Select(jsonElement => jsonElement.GetProperty("msg").GetString())
								.ToArray();
		...
		private static async Task<PaginatedResponse<T>> AsPaginatedResponse<T>(this HttpContent httpContent, string propertyName, JsonSerializerOptions options = null, CancellationToken cancellationToken = default)
		{
			...
			var pageNumber = rootElement.GetPropertyValue("page", 0);
			var pageSize = rootElement.GetPropertyValue("per_pagee", 0);
			var totalRecords = rootElement.GetPropertyValue("count", 0);
