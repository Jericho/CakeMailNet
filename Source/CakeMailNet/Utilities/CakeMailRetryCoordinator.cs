...
namespace CakeMailNet.Utilities
{
	...
	internal class CakeMailRetryCoordinator : IRequestCoordinator
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="CakeMailRetryCoordinator" /> class.
		/// </summary>
		/// <param name="tokenHandler">The handler that takes care of renewing expired tokens.</param>
		public CakeMailRetryCoordinator(ITokenHandler tokenHandler)
		{
		...
		/// <param name="tokenHandler">The handler that takes care of renewing expired tokens.</param>
		public CakeMailRetryCoordinator(IRetryConfig config, ITokenHandler tokenHandler)
			: this(tokenHandler)
		{
		...
		/// <param name="config">The retry configuration.</param>
		/// <param name="tokenHandler">The handler that takes care of renewing expired tokens.</param>
		public CakeMailRetryCoordinator(IEnumerable<IRetryConfig> config, ITokenHandler tokenHandler)
			: this(tokenHandler)
		{
			_defaultRetryCoordinator = new RetryCoordinator(config);
		}
		...
		public async Task<HttpResponseMessage> ExecuteAsync(IRequest request, Func<IRequest, Task<HttpResponseMessage>> dispatcher)
		{
			...
			if (requestRedirected && requestIncludesAuthorization)
			{
				...
				// header which causes the API to reject the request. The solution is to
